...
- Recipe authoring docs: `custom_recipes/README.md` and both V10 guides now explain how the `layers` array values are derived - the 12-chunk map from the empirical sweeps, the built-in family tables, the `clamp(strength x shape x gain, +/-6)` landing math, a step-by-step derivation procedure with a copy-paste helper (the same scaling as the card's Structure/Finish dials), and a worked example.
