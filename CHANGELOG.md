...
real Krea 2 model. V9 behavior is completely unchanged.

- **`suggest the visual style` now does true style transfer.** In 0.2.1 the
  V10 style recipe mostly shared the reference's palette, like a color
  grade. It now carries the reference's medium, palette, brushwork
  character, and rendering finish, while an anti-blur `focus` line keeps
  the recipe's deliberate reference-softening from being read back as the
  style - so subjects stay crisp even with watercolor, gouache, or
  soft-photo references. Capped at `0.65`; lands clearly from about `0.55`.
  For the strongest content-plus-style results, pair a `keep the same
  subject` card at `0.9` with the style card at `0.55`.
- **`copy lighting and mood` now lands dramatic light.** Previously a
  moody-sky reference arrived as flat location color; the recipe now
  studies the reference so the broad light and cloud masses survive, with a
  `focus` that keeps the reference's place and objects out. Storm mood
  lands at `0.65`, full drama by `0.9`.
- **`copy big shapes only` keeps your colors.** Silhouette guidance
  previously tended to pull results toward gray; a structure-only layer
  table now delivers the massing while the prompt's natural colors stay.
- **`suggest material or texture` carries real surface character** -
  textile ornament, fabric weave, ceramic glaze, carved relief - instead of
  mostly recoloring. Capped at `0.65`; stay near `0.55` when the object's
  exact form must not move.
- **Two new starter recipes** in `custom_recipes/starter-pack.yaml`:
  `borrow photo finish` (photographic tone, contrast, grain, and color
  grade from a photo reference; lands clearly from about `0.55`) and
  `borrow drawing medium` (a deliberately gentle monochrome paper-and-ink
  tonal shift; its description states honestly that a full drawn look
  needs the prompt's cooperation - louder settings would import the
  drawing's subject).
- **Fifteen more bundled recipes in two new packs**, every one
  render-validated end to end under the stack's default smart timing.
  `custom_recipes/designer-artwork-pack.yaml` - ten recipes for designers
  working from artwork: `borrow the poster style`, `borrow the soft media
  look`, `borrow the pattern energy`, `borrow the era print look`,
  `borrow the paper and canvas`, `borrow the metallic accents`,
  ...
  `custom_recipes/edit-composite-pack.yaml` - five recipes for editing
  and compositing: `match the scene light`, `match the monochrome look`,
  `match the atmosphere`, `use the background only`, and `carry the
  subject over`. Each recipe's dropdown description doubles as its usage
  guide: the strength to start at, which references work best (full-bleed
  artwork for the style family, scene plates for the light matchers,
  clean product shots for subject carrying), and what to expect when a
  reference fights the recipe. Delete or underscore a pack file to remove
  its recipes.
- **The Recipe Kit README now documents the smart-timing tuning rule**
  learned from the packs' render series: the `early` multiplier is the
  structure-phase volume, motif-scale borrows (patterns, ornament,
  borders) need `early` around `0.85` to arrive, and recipe validation
  should always run with the stack's default `smart per-card timing`
  (the constant mode ignores `early`/`late` and can overstate a recipe).
- **The Recipe Builder generates recipes that match the new tuning**,
  including the style and lighting focus texts and the new layer tables;
  every builder output is schema-verified through the pack's real recipe
  loader.
- **The full V10 demo set was re-rendered** on the new tuning (every demo
  PNG still embeds its exact workflow), and the README's strength guidance
  now matches the calibrated bands: appearance recipes land clearly from
  about `0.55`-`0.65` and whisper below `0.5` by design; structure and
  identity work at `0.9`-`1.2`.
- Documentation refreshed throughout the V10 user guides, technical paper,
  node pages, testing guide, docs index, and the Recipe Kit README -
  including honest notes on what the conditioning cannot do (no
  brushwork-repaint onto surfaces, no literal scene-behind-subject
  compositing).
...
- Card: the `focus` recipe field - a custom recipe can now study **one named aspect** of its reference ("the clothing and garment style worn by the person, not the person's identity, face, or the background"). The text is written into the vision encoder's instruction line for that card (`study only ... ignore everything else about it`; away cards get scoping language), the guide packet carries it as an append-only key, and the stack report prints it on the card's line. Render-verified on Krea 2 with same-seed A/B pairs: aspect *selection* works (a clothing-focused recipe kept the reference's garment and gained garment detail) and aspect *de-selection* is remarkably strong (focusing the same photo on "the background, not the person or their clothing" removed the reference's signature red coat entirely) - while scene-wide moods such as weather ride the image itself, so the docs steer `focus` toward object-bound aspects. Strings are whitespace-normalized and capped at 300 characters; V9 workflows are unaffected.
- Starter recipes: `custom_recipes/starter-pack.yaml` ships **enabled** with three render-validated recipes that appear in `Use image for` out of the box - `borrow the weather` (strong blur + low detail bring a reference's storm/fog/golden-hour mood over without its buildings or poles), `borrow the clothing style` (a `focus` recipe; its description documents the inverse trick), and `cinematic color grade` (a bolder palette sibling, capped at 1.2). Delete or underscore the file to opt out.
- Recipe Builder: an optional "Focus on one aspect" box (step 2) writes the `focus` field with the node's exact normalization, with honest guidance on where focus is strong; all builder outputs remain machine-verified through the real recipe loader.
- The V10 technical paper was rewritten as a **standalone technical reference** (`docs/krea-v10-technical-paper.md`): every widget of both V10 nodes with ranges and defaults, the full conditioning math (the twelve text-encoder taps, mute-and-diff deltas, per-band compose, the inert pooled channel), the strength model (feel curves, caps, timing, direction, balance), the image-preparation pipeline with exact kernel sizes, the prompt machinery including the multilingual text/logo guard and the new focus lines, the complete retuned recipe tables, custom recipes and the starter pack, the study cache, both feedback outputs, V9 interoperability, the frozen surfaces, and a worked end-to-end example - with no dependency on the V9 paper.
- The custom-recipes README is now the **Recipe Kit** hub: the Builder, the auto-loaded starter pack, the disabled template, the schema/mechanics guide (including a dedicated `focus` section), and the technical reference, indexed in one table.
- New example workflow `krea-v10-starter-recipe-workflow.json`: a shipped custom recipe in action (`cinematic color grade` from the starter pack grading the scene from the bundled style reference; render-validated). The workflow READMEs and the main README list it.
- Documentation sync pass: the main README's "What V10 Adds" now covers `focus`, the starter recipes, and the Recipe Builder; the V10 user guide gained a "Focus: recipes that study one aspect" section (with the render-verified select/de-select evidence and limits), matching "What V10 Adds" rows, and troubleshooting entries; the V10 documentation index was refreshed (Recipe Kit and Builder in Start Here, the full twenty-demo journey list, no more "pending render" notes); the stack encoder node page points at the standalone V10 technical reference and mentions the report's focus line; and the testing guide's V10 smoke checklist covers the starter recipes, focus report lines, and the Builder, with the demo-set section describing the current twenty-journey gallery and its regenerator script.
...
- Recipe Builder (`web/recipe-builder.html`): a single-page, dependency-free HTML utility that turns three plain-language questions (name it, what should the reference give your picture, how strongly) into a validated, ready-to-drop-in recipe file - no knowledge of treatments, pulls, or the layers schema required. Twelve artist-worded jobs map to the render-validated recipe bundles; a loudness choice (Whisper / Clear / Bold) scales the tuning within its calibrated band; optional fine-tuning exposes the subject policy, the structure/finish emphasis dials (the same math as the card's manual layer dials), and a strength ceiling. Each job states honestly what it cannot do, the output panel previews YAML or JSON with download/copy plus install steps and the two-minute render check, and reserved built-in names are rejected as you type. Ships with the pack and is served by ComfyUI at `/extensions/<pack folder>/recipe-builder.html`; linked from the custom-recipes README, both V10 guides, and the card's node page. Every intent x loudness (x fine-tune) output was machine-verified through the pack's real recipe loader, and a builder-generated recipe was render-tested end-to-end on the live node.
